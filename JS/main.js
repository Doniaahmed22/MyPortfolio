...
let roleIndex = 0; // الجملة الحالية
let charIndex = 0; // الحرف الحالي
let isDeleting = false; // حالة الحذف

function typeEffect() {
    const currentRole = roles[roleIndex]; // الجملة الحالية
    ...
    // تحديث النص المعروض
    typingEffect.textContent = currentRole.substring(0, charIndex);

    // الانتقال إلى الجملة التالية عند الانتهاء
    if (!isDeleting && charIndex === currentRole.length) {
        isDeleting = true; // ابدأ الحذف
        setTimeout(typeEffect, 1000); // انتظر قبل الحذف
    } else if (isDeleting && charIndex === 0) {
        isDeleting = false; // ابدأ الكتابة للجملة التالية
        roleIndex = (roleIndex + 1) % roles.length; // انتقل للجملة التالية
        setTimeout(typeEffect, 500); // انتظر قبل البدء في الجملة التالية
    } else {
        setTimeout(typeEffect, isDeleting ? 100 : 200); // سرعة الكتابة والحذف
    }
}
